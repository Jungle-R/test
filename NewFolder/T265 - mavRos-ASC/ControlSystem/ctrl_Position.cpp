...
/*控制接口*/
	...
	/*水平位置*/
		...
		/*直线飞行*/
			...
			bool Position_Control_set_TargetPositionXYBodyheading( double posx, double posy,  double vel, double TIMEOUT )
			{
				Quaternion att;
				get_Attitude_quat( &att );
				double yaw = att.getYaw();
...
{	
	...
	bool inFlight;	get_is_inFlight(&inFlight);
	vector3<double> Position;	get_Position_Ctrl(&Position); //时刻更新现在的速度
