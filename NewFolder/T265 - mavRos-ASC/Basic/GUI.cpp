...
static void GUI_Server(void* pvParameters)
{
	...
	while(1)
	{
		...
		/*姿态*/
			...
			//roll
			sprintf( str, "%3.1f",  msg_OLED.params[3] );
			DrawFont16x8( str, 48, 49, WHITE );
			//pitch
			sprintf( str, "%3.1f",  msg_OLED.params[4] );
			DrawFont16x8( str, 64, 49, WHITE );
			//yaw
			sprintf( str, "%3.1f",  msg_OLED.params[5] );
		...
		/*Z轴*/
			...
			sprintf( str, "%3.1f", msg_OLED.params[6]);
			...
			if( get_Health_Z(&posinfz) )
			{
				...
				health = 0;
			sprintf( str, "%3.0f%%", msg_OLED.params[1] );
			DrawFont16x8( str, 32, 49, WHITE );
		/*Z轴*/
			
		/*XY轴*/
			//Vx
			sprintf( str, "%3.1f",msg_OLED.params[7] );
			DrawFont16x8( str, 128, 49, WHITE );
			//Vy
			sprintf( str, "%3.1f", msg_OLED.params[8] );
			...
			if( get_Health_XY(&posinfxy) )
			{
				...
				health = 0;
			sprintf( str, "%3.0f%%", msg_OLED.params[0]);
