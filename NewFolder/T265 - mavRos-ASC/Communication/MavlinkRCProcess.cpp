...
extern bool xyz_lock ;

void Mav82_SET_ATTITUDE_TARGET (  M35_mavlink_message msg_rd )
{
	 
	 Quaternion q;
		Quaternion(msg_rd.params[0],msg_rd.params[1],msg_rd.params[2],msg_rd.params[3]);
	
     Position_Control_Disable();

     Attitude_Control_set_Target_RollPitch(q.getRoll(),q.getPitch());
     Attitude_Control_set_Target_Yaw(q.getYaw());
     float thrust=constrain(msg_rd.params[4]-0.5,0.5);
...
static void Msg82_SET_ATTITUDE_TARGET ( uint8_t Port_index, const mavlink_message_t* msg )
{
 const mavlink_set_attitude_target_t* msg_rd = (mavlink_set_attitude_target_t*)msg->payload64;
	M35_mavlink_message M35_msg;
	
	M35_msg.msg_num = 82;
	M35_msg.params[0] = msg_rd->q[0];
	M35_msg.params[1] = msg_rd->q[1];
	M35_msg.params[2] = msg_rd->q[2];
	M35_msg.params[3] = msg_rd->q[3];
	M35_msg.params[4] =	msg_rd->thrust;
	
	SendMsgToM35(M35_msg,0);
	

    
}

M35_mavlink_message msg_OLED;

#define Relative_CS 0
#define Absolute_CS 1

// yaw  角度 制
// 前左上 
 //2021-6-3 step_xyz,step_yaw由函数中 static 变量改为 全局变量
 uint8_t step_xyz=0;
	uint8_t step_yaw=0;
void Mav84_SET_POSITION_TARGET_LOCAL_NED (  M35_mavlink_message msg_rd )
{
//	static uint8_t step_xyz=0;
//	static	uint8_t step_yaw=0;
	
//		M35_msg.type_mask = msg_rd-> type_mask;
				//1|2|4|8|64|128|256|512|2048;
				//  12    x y z & yaw is relative
	
				float forward= msg_rd.params[0]*100;
				float left= msg_rd.params[1]*100;
				float up= -msg_rd.params[2]*100;
				float forward_vel= msg_rd.params[3]*100;
				float left_vel= msg_rd.params[4]*100;
				float up_vel= -msg_rd.params[5]*100;
				float yaw=msg_rd.params[6]; //yaw
				float yaw_rate=msg_rd.params[7];//yaw rate
				...
				if((msg_rd.type_mask&POSITION_TARGET_TYPEMASK_X_IGNORE)||(msg_rd.type_mask&POSITION_TARGET_TYPEMASK_Y_IGNORE)||(msg_rd.type_mask&POSITION_TARGET_TYPEMASK_Z_IGNORE))		
				{
						...
						{
							if((forward_vel!=0)||(left_vel!=0))
									Position_Control_set_TargetVelocityBodyHeadingXY_AngleLimit(forward_vel,left_vel);//Position_Control_set_TargetVelocityBodyHeadingXY_AngleLimit
							else
								Position_Control_set_XYLock();	
						}
						
						//***************************Vz****************************//	
						if(!(msg_rd.type_mask&	POSITION_TARGET_TYPEMASK_VZ_IGNORE))
						{
							if(up_vel!=0)
				...
				{			
						if((forward!=0)||(left!=0)||(up!=0)) //xy 不为0
						{
							if (step_xyz==0)
							{
								if(msg_rd.coordinate_frame==Absolute_CS)
								{
								  Position_Control_set_TargetPositionXYBodyheading(forward,left,40); //		
									Position_Control_set_ZLock();	
									 step_xyz = 1;		
								}									
								else 
									if(  Position_Control_set_TargetPositionXYZRelativeBodyheading (forward,left,up,50) )									
										step_xyz = 1;		
							}
			...
			//********************Yaw***********************//				
				if(!(msg_rd.type_mask&	POSITION_TARGET_TYPEMASK_YAW_IGNORE))
				{
						...
						else 
						{
							if (step_yaw==0)
							{
								Attitude_Control_set_Target_YawRelative(degree2rad(yaw));
								step_yaw = 1;						
							}
							else if(step_yaw==1)
							{
...
static void Msg84_SET_POSITION_TARGET_LOCAL_NED ( uint8_t Port_index, const mavlink_message_t* msg )
{
	...
	//bodyhangding 前左上
		 M35_msg.params[0]=(msg_rd->y);
		 M35_msg.params[1]=(msg_rd->x);
		 M35_msg.params[2]=(msg_rd->z);
		 M35_msg.params[3]=(msg_rd->vy);
		 M35_msg.params[4]=(msg_rd->vx);
		 M35_msg.params[5]=(msg_rd->vz);
		 M35_msg.params[6]=msg_rd->yaw;
		 M35_msg.params[7]=msg_rd->yaw_rate;
	
		M35_msg.type_mask = msg_rd-> type_mask;
		M35_msg.coordinate_frame = msg_rd-> coordinate_frame;
	
	SendMsgToM35(M35_msg,0.01);
...
static void Msg103_VISION_SPEED_ESTIMATE ( uint8_t Port_index, const mavlink_message_t* msg )
{
    const mavlink_vision_speed_estimate_t* msg_rd = (mavlink_vision_speed_estimate_t*)msg->payload64;
				...
				vel_vision.x = (msg_rd->x)*100.0;
				vel_vision.y = (msg_rd->y)*100.0;
				vel_vision.z = 0;
			...
			PositionSensorUpdateVel(default_vision_XY_Velocity_index,vel_vision,true);//,0,0,0,0,0.1);
			PositionSensorUpdatePosition(default_vision_Z_Position_index,pos_vision,true);//,0,0,0,0,0.1);
	
//	PositionSensorUpdateVel( default_vision_XY_Velocity_index,vel_vision,true, 0,0, 0, 0, 0 );
//	PositionSensorUpdateVel( default_vision_Z_Position_index,pos_vision,true, 0, 0, 0, 0, 0);	
